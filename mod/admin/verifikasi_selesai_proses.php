...
<div class="container" style="margin-top:30px; padding:20px;">
    <div class="row" style="margin-bottom:10px;">
        <div class="col-md-12 col-xs-12" style="padding-left:7px;">
            <h3 style="margin:0px;">Verifikasi Selesai Pengurusan</h3>
        </div>
    </div>  
...
<script type="text/javascript">
    ...
    $(document).ready(function(){
         $("#jum_data").load(url,"op=jum_data_sp");
         $("#bodi_tabel").load(url,"op=load_tabel_sp");
    });
    ...
    function selesai(id_pengurusan){
        ...
        if(konfir == true){
            $.ajax({
                url : url,
                data : "op=selesai&id_pengurusan="+id_pengurusan,
                success:function(msg){
                    if(msg == "sukses"){
                        alert("Berhasil Verifikasi");
                        $("#bodi_tabel").html("<div style='margin-top:80px;'><center><i class='fa fa-spinner fa-spin fa-5x fa-fw'></i><h3>Sedang Mempersiapkan Halaman</h3></center></div>");
                        $("#bodi_tabel").load(url,"op=load_tabel_sp");
                        $("#jum_data").load(url,"op=jum_data_sp");
                    }else{
